...
const
      {WordBitLength ist die Länge eines Word in Bitdarstellung}
      WordBitLength = (SizeOf(Word) * 4); {8 Bit}
      {DWordBitLength ist die Länge eines DWord in Bitdarstellung}
      DWordBitLength = (SizeOf(DWord) * 4);

type
  {TScreens wird von @link(ScreenToPercent) und @Link(PercentToScreen) verwendet,
  um Breite und Höhe des Bildschirms zu speichern}
  TScreens = record
    Width, Height: Word;
  end;

  {TScreens wird von @link(ScreenToPercent) und @Link(PercentToScreen)  verwendet,
  um Breite und Höhe des Bildschirms in Prozent zu speichern}
  TPercent = record
    x, y: Double;
  end;


{BitLinkByte verknüpft Bit-weise zwei Bytes zu einem Word
Z.B. Integer in einer Binär-Datei sind immer in zwei Bytes (die vertauscht sind) gespeichert : D007
     Diese zwei Bytes können aber nicht ohne weiteres in einen Integerwert transferiert werden
     Deshalb wird Bit1 $D0 und Bit2 $07 zugewiesen um einen Wordwert (2000) zu resultieren

Hinweis : In Binär-Dateien sind die Zahlenwerte in der Hälfte ihrer Anzahl von Bits vertauscht :
          Bsp : Binär : 10101111 01000000(2 Bytes) , als Integerwert zu lesen = 01000000 10101111 oder BitLinkByte(64,175) =  16559 (Word)
         Steht in der Datei z.B.  $12 $23 $45 $67  , so entspricht dies nicht dem Aufruf von
                                                     ...
                                                     sondern , da $12 $23 und $45 $67 vertauscht werden müssen :
...
{Transferiert vier Byte-werte zu einem Dword , dabei werden Bit1 und Bit2 mit Bit3 und Bit4 vertauscht und verknüpft wiedergegeben
...
WordToBytes liefert das höher und niederwertige Byte von Worth zurück
Diese Funktion ist identisch mit dem Aufruf von Lo für Bit2 und Hi für Bit1
mehr dazu siehe @Link(BitLinkByte)
}
...
{GetBit gibt den Status (gesetzt oder nicht) eines bestimmten Bits (BitNo) von Combination zurück
Die Zählung fängt bei 0 an und hört bei 63 (64Bit bei Int64) auf
Es ist allgemein so ,dass das Zählen beim Bits rechts und von 0 anfängt.
Das erste bit von links ist also das 7 Bit.
mehr dazu siehe @Link(BitLinkByte)
}
function GetBit(Combination: Int64; BitNo: Byte): Boolean;

{ggT ermittelt (rechnerisch) den größten gemeinsamen Teiler zweier Zahlen}
function ggT(a, b: integer): integer;

{kgV ermittelt (rechnerisch) den kleinsten gemeinsamen Teiler zweier Zahlen}
function kgV(a, b: Integer): Integer;

{SetOrgRect setzt die Höhe und Weite als Koordinaten um, d.h.
 R.Bottom := Y+Height
 R.Right  := X+ Width}
procedure SetOrgRect(var r: TRect; x, y, width, height: Integer);


{ScreenToPercent berechnet aus einer Größe Screen und dem Anteil Pos den Prozentwert
Sie wird dazu verwendet, absolute Koordinaten auf dem Bildschirm unabhängig zu anderen
Auflösungen zu machen, um so immer z.B. Fenstergrößen und Positionen zu wahren.
}
function ScreenToPercent(Screen: TScreens; Pos: TPoint): TPercent;
{PercentToScreen berechnet aus Prozentwerten (Perc) und den dazugehörigen Größenangaben Screen,
...
begin
  Result := Bit2 shl
    (sizeof(Word) * 8); //Länge eines Word-Wertes = 2Bytes |1 Byte = 8 Bits
