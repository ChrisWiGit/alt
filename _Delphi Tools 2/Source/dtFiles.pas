...
type //R�ckgabewert mu� die Anzahl der geschriebenen Bytes sein
...
//Ist GetDrivesEx vorzuziehen , wenn man alle Laufwerke aufgez�hlt haben m�chte.
...
//Gibt die Erstellungs,letzte Zugriffs- und Ver�nderungszeit + datum zur�ck
...
//Wie zuvor nur kann hier der Dateiname direkt angegeben werden
...
//Gibt die Gr��e der Datei in Bytes zur�ck
function GetFileSize(const FileName: string): longint;
function GetFileSize_hfile(fileHandle: HFILE): Longint;
//Gibt noch mehr Informationen �ber die Datei zur�ck
...
//Liest den Typ der Datei Filename aus.
...
//Gibt Festplattenspeicherdaten zur�ck
function GetDiskFreeSpace(drive: char; FreeSpaceRec: TFreeSpaceRec): Boolean;
//Gibt True zur�ck , wenn die Datei oder das Verzeichnis (auch Laufwerk)
//existiert
...
//Erweitert einen kurzen Dateiname zu einem langen Dateinamen
function GetFullFileName(const ShortName: string): string;
//Liefert alle Verzeichnisname in einer Stringliste zur�ck
...
//Gibt die Anzahl der Dateien mit dem Namen FileName im
...
//Wie IsFileInDir , nur eben mit Verzeichnissen anstatt Dateien
function IsDirInDir(const DirName, PathName: string): Integer;


//  Names = Array of String;
//L�scht Dateien in den Papierkorb , wenn AllowUndo = TRUE
...
//Wie DeleteFile nur mit TStrings f�r TOpenDialog.Files-�bergabe
...
//Liefert ein konstantes Systemverzeichnis zur�ck
function GetSpecDirectory(Typ: TTypeDirectory): string; overload;

//Erzeugt eine Tempor�re Datei in PathName mit einem Prefix (Prefixstring)
...
//wie AddDirToFile , nur wird die Erweiterung ge�ndert anstatt das Verzeichnis
function AddExtToFile(const FileName: string; Extension: string; OverWriteExisting: Boolean): string;

//Gibt FileName zur�ck wenn dieser existiert sonst NewFileName
function ReplaceInValidFile(const FileName, NewFileName: string): string;

//Erstellt eine leere Datei mit dem Namen FileName
...
//ermittelt alle verf�gbaren Laufwerke
//und gibt die Laufwerksbuchstaben in einer Stringliste zur�ck
function GetLogicalDrives: TStringList;

//ermittelt den DOS-Pfadname aus einem langem Verzeichnissname
function ExtractShortPathName(const FileName: string): string;
//ermittelt aus einem DOS-Pfadname einen langem Verzeichnissname
function ExtractLongPathName(const PathName: string): string;


//schreibt an das Ende einer Datei beliebiege Informationen
...
//Tested , ob einen Datei bereits ge�ffnet wurde und liefert
...
//Pr�ft , ob ein Dateiname ung�ltige Zeichen enth�lt (FALSE)
function IsFileNameValid(FileName: string): Boolean;


//Liefert das Verzeichnis (mit "\" am Ende) der Anwendung zur�ck
function GetApplicationPath: string;

//Ermittelt aus einem Pfad , den n-ten (Level) Pfadname
...
//1.305
//Erstellt alle noch nicht vorhanden Unterverzeichnisse
