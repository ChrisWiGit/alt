{
@abstract(DXPlay.pas beinhaltet Funktionen f�r den Umgang mit DirectPlay)
...
//!! It will cause an error , if DXPlay is not initialized ,so we have to check it.
              if FDPlay = nil then
              begin
                {FreeMem(lpvMsgBuffer);}
                FInThread := False; //get out here.
                exit;
              end;
            end;
          end;
...
begin
  ...
  try
    ...
    if FInThread then
      ...
      FRecvThread.Free;
    if FRecvEvent[0] <> 0 then //!! added by author
...
begin
  ...
  begin
    ...
    FDPlay.SendEx(FLocalPlayer.ID, ToID, dwFlags, Data^, DataSize,
      0, 0, nil, @Result); // 0 �ȊO�̓T�|�[�g���Ȃ��f�o�C�X����̂Ŏg��Ȃ�
end;
