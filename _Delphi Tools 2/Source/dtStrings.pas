{
@abstract(dtStrings.pas beinhaltet Funktionen f�r den Umgang mit Zeichenkettent)
...
const
      {Standard DoubleWord - Stringhexformat}
      StrHexFormatStd = '%s-%s';
      StrHexFormat: string = StrHexFormatStd;
      {Ein Array, der alle Hex-Zeichen beinhaltet}
      HexArray: array[0..15] of char = '0123456789ABCDEF';

type  T16HexBit = string[2];

//
type {TCallCompareString wird von @Link(FindStringCall) verwendet, um benutzerdefiniert in einer StringListe
     zu suchen.
     Index ist der aktuelle Indexwert in der Stringliste. Stringname der Text an der Stelle Index.
     StrObject bezeichnet den Data-Wert (kann nil sein) und CompareWithStr ist der Text, welcher FindStringCall �bergeben wurde,
     um nach ihm zu suchen.}
     TCallCompareString = function(Index: Integer; StringName: string; StrObject: TObject;CompareWithStr: string): Integer;

type
   {TEnumCharRec wird von @Link(EnumChars) und @Link(GetCharCount) verwendet, um den R�ckgabewert zu bestimmen.
   CharCount entspricht der Anzahl von einzelnen Zeichen, Count der Gesamtzahl aller Zeichen.
   SingelCharCount ist die Anzahl von Zeichen , ohne da� doppelte mitgez�hlt worden sind/ f�r Zahlen : = 0}
   TEnumCharRec = record
    CharCount: array of Longint; //Anzahl von einzelnen Zeichen/gefundene Zahl
    Count, //Gesamtanzahl der Zeichen/der Zahlen
      SingleCharCount: Longint; //Anzahl von Zeichen , ohne da� doppelte mitgez�hlt worden sind/ f�r Zahlen : = 0
  end;
...
type
  {TOnDataWrite wird von @Link(SaveStringsToStream) verwendet, um benutzerdefinierte Daten mit in den Stream zu speichern.  }
  TOnDataWrite = function(Stream: TStream; Index: Integer; aObject: TObject): Longint of object;
  {TOnDataWrite wird von @Link(LoadStringsFromStream) verwendet, um benutzerdefinierte Daten mit aus dem Stream zu laden.  }
  TOnDataRead = function(Stream: TStream; Index: Integer; var aObject: TObject): Longint of object;


{DelSpaces l�scht alle Leerzeichen (#255), die in dem String Str vorkommen.
Der R�ckgabewert ist dann der String ohne Leerzeichen.}
function DelSpaces(Str: string): string;


{ChangeChar vertauscht alle Zeichen ch im String str mit Toch.}
function ChangeChar(str: string; ch, toch: Char): string;

{GetTransString kehrt einen String um. ABCD -> DCBA}
function GetTransString(str: string): string;

{GetTransString ersetzt in einem Text den String ReplaceText , unter
Ber�cksichtiung der Gro�- und Kleinschreibung ,durch den Text NewText}
function ReplaceStr(const Text, ReplaceText, NewText: string): string;
{ReplaceLStr ersetzt ,wie @Link(ReplaceStr) einen Text durch einen anderen, nur ohne Ber�cksichtigung der Gro�- und
Kleinschreibung}
function ReplaceLStr(const Text, ReplaceText, NewText: string): string;


{Hex8ToString wandelt ein Hex-Char-Wert in einen Integer-Wert um.
M�gliche Werte sind in @Link(HexArray) enthalten.}
function Hex8ToString(Hexchar: char): Integer;

{Hex16ToString wandelt einen 2-Hex-Char-Wert in einen Integer Wert um.
M�gliche Werte sind in @Link(HexArray) enthalten.}
function Hex16ToString(Hexchar: T16HexBit): Byte;

{FormatHexStr wandelt eine DWord in eine formatierte Hexdezimalzahl als String um.
Die Art der Darstellung kann in @Link(StrHexFormat) ge�ndert werden.
Jeder String in StrHexFormat ist 4 Zeichen lang (0..9,A..F)}
function FormatHexStr(L: DWORD): string;

{FormHexStr, siehe @link(FormatHexStr)
-- ??}
function FormHexStr(L: DWORD): string;


{Hex8 wandelt einen 32-Bit-DWORD-Wert in einen hexdezimalen Wert um (max 8Hexzahlen)
Max DWORD :  4294967295 -> FFFFFFFF (ohne Vorzeichen!)}
function Hex8(L: DWORD): string;

{Hex4 wandelt ein vorzeichenlosen 16-Bit-Word-wert in einen hexdezimalen Wert um (max 4Hexzahlen)
Max Word : 65535 -> FFFF}
function Hex4(W: Word): string;
{Hex2 wandelt ein vorzeichnlosen 8-Bit-Byte-wert in einen hexdezimalen Wert um (max 2Hexzahlen)
Max Byte : 255 -> FF}

{Hex2 wandelt einen Byte-Wert in eine Hexdezimale Zahl um.
Dezimal - Hexdezimal
     01 - 01
     02 - 02
     ..
...
{DoubleZero wandelt eine einstellige Zahl (Bsp : 1) in eine zweistellige : 2 -> 02}
function DoubleZero(str: string): string;
{DoubleZeroInt wandelt eine einstellige Zahl als String (Bsp : 1) in eine zweistellige : 2 -> 02}
function DoubleZeroInt(str: Integer): string;

{Double2ZeroInt - noch nicht kommentiert -- ??}
function Double2ZeroInt(str: Integer; DoIt: Boolean): string;




{FindString sucht in einer StringListe nach dem Vorkommen von Str, ohne Gro�- und Kleinschreibung zu beachten.
Der R�ckgabewert ist der Index, des ersten gefundenen Strings in der Stringliste.
Bei nicht-vorkommen wird -1 zur�ckgeliefert.}
function FindString(Strings: TStrings; Str: string): Integer;

{FindStringCall sucht benutzerdefiniert in einer StringListe nach dem Vorkommen von Str, ohne Gro�- und Kleinschreibung zu beachten.
Der R�ckgabewert ist der Index, des ersten gefundenen Strings in der Stringliste.
Bei nicht-vorkommen wird -1 zur�ckgeliefert.}
function FindStringCall(Strings: TStrings; Proc: TCallCompareString; FindStr: string): Integer;


{ConvertStringToList splittet den String Str in mehrere einzelne Strings bei dem Zeichen NewLineChar auf
und gibt sie in einer StringListe zur�ck
Feature : NewLineChar mu� nicht als letztes Zeichen gesetzt sein}
function ConvertStringToList(const Str: string; const NewLineChar: Char): TStringList;


{ConvertSemiColonStrToStringList kovertiert einen String , welcher als Stringbegrenzungen das ";" enth�lt in eine Stringliste
siehe auch @Link(ConvertStringToList)
}
function ConvertSemiColonStrToStringList(aString: string): TStringList;

{FindInString sucht nach fast beliebigen Typen in einem String "Source".

In FindStrings k�nnen nach folgende Typen gesucht werden :
 Integer , Boolean , Char , String
 Bsp . : ..('123 TRUE FalSCH c, FALSE hallo',[123,TRUE,FALSE,'c','hallo',FALSE)
         ergibt : 1 (f�r 123)
 boolean - Ausdr�cke werden in Deutsch und Englisch verstanden (WAHR,FALSCH,TRUE,FALSE)
 CaseSensitive gibt an ob zwischen Gro�- und Kleinschreibung unterschieden werden soll (TRUE)
Der R�ckgabewert ist die Anfangsposition des zuerst gefundenen Strings
wurde keine �bereinstimmung gefunden , wird 0 zur�ckgeliefert}
...
{IntToBinStr erstellt einen String aus den Bits (Bits) der nur Count Bits enth�lt
Bsp : 5 -> 101}
function IntToBinStr(Bits: Int64; Count: Byte): string;

{BinStrToInt erstellt einen Integerwert aus einem BitString (BitStr)
Es werden nur 1 f�r Bit gesetzt und 0 f�r nicht gesetzt als Bits erkannt
Auserdem wird nur bis zu dem ersten ung�ltigen Zeichen (oder StringEnde)
die Bitkombination erstellt}
function BinStrToInt(BitsStr: string): Int64;

{ReverseString dreht einen String um, und liefert diesen dann zur�ck.}
function ReverseString(Str: string): string;

{DeleteCharsFromString entfernt aus einem Text alle Zeichen, die in Chars vorkommen,
und liefert das Resultat zur�ck.}
...
{EnumChars z�hlt in einem String die Zahl des Auftretens aller Zeichen zusammen.
Der R�ckgabewert ist die Anzahl der Zeichen und eine spezifierte Aufgliederung in EnumCharRec.
Um den Inhalt von EnumCharRec zu verstehen, wird die Funktion @Link(GetCharCount) verwendet.
}
function EnumChars(Text: string; var EnumCharRec: TEnumCharRec): Integer;
{GetCharCount ermittelt die Anzahl eines Zeichens aChar im String zur�ck.
Die Berechnung wurde zuvor in EnumChars durchgef�hrt und mu� GetCharCount �bergeben werden.
Der R�ckgabewert ist dann die Anzahl des gew�nschten Zeichens.}
function GetCharCount(aChar: Char; EnumCharRec: TEnumCharRec): Integer;

{EnumNumbers sucht aus einem String alle ganzzahligen Werte (positive und negative)
der R�ckgabewert entspricht dessen Summe}
function EnumNumbers(Text: string; var EnumCharRec: TEnumCharRec): Integer;




{StringToTCharArray konvertiert einen String in einen Array of char}
function StringToTCharArray(Chars: string): TCharArray;

{EnumSpecChars gibt die Gesamtanzahl der in Chars angegebenen und in Text gefundenen Zeichen zur�ck}
function EnumSpecChars(Text: string; const Chars: TCharArray): Integer;

{_EnumSpecChars l��t die eckige Klammerschreibweise f�r Chars zu (Bsp : ['s','d'])}
function _EnumSpecChars(Text: string; const Chars: array of char): Integer;

{BOOLToStr liefert 'TRUE' oder 'FALSE' als gleichnamigen String zur�ck.}
function BOOLToStr(b: Boolean): string;
{BOOLToStrExt liefert sTRUE oder sFALSE zur�ck, wenn b einen gleichnamigen Wert annimmt.}
function BOOLToStrExt(b: Boolean; sTRUE, sFALSE: string): string;



{SaveStringsToStream speichert eine StringListe in einen Stream und liefert die Anzahl der geschrieben Bytes zur�ck.
Benutzerdaten k�nnen in OndataWrite geschrieben werden.}
function SaveStringsToStream(StringList: TStrings; Stream: TStream; OnDataWrite: TOnDataWrite = nil): Longint;

{LoadStringsFromStream l�dt eine StringListe in einen Stream und liefert die Anzahl der gelesenen Bytes zur�ck.
Benutzerdaten k�nnen in OnDataRead geschrieben werden.}
function LoadStringsFromStream(StringList: TStringList; Stream: TStream; OnDataRead: TOnDataRead = nil): Longint;


{MultiReturn wiederholt den String @Link(Return) "times"-Mal}
...
begin
  ...
  raise Exception.CreateFmt('Hex8ToString : %s ist kein g�ltiger 8Bit Hex-Wert!', [HexChar]);
end;
...
begin
  ...
  if Length(HexChar) = 0 then
    raise Exception.Create('Hex16ToString : Ung�ltiger Parameter : <Leerstring>.');
